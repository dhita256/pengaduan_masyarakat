...
def index():
    cursor.execute("""
        SELECT p.id, p.nama, p.isi, p.tanggal, t.id, t.tanggapan, t.tanggal
        FROM pengaduan p
...
def tambah():
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO pengaduan (nama, isi) VALUES (%s, %s)", (nama, isi))
        db.commit()
...
def dashboard():
    if 'admin' not in session:
        return redirect(url_for('login'))
    cursor.execute("SELECT * FROM pengaduan LEFT JOIN tanggapan ON pengaduan.id = tanggapan.pengaduan_id")
    data = cursor.fetchall()
...
def delete(id):
    ...
    cursor.execute("DELETE FROM tanggapan WHERE pengaduan_id=%s", (id,))
    db.commit()
...
@app.route('/admin/tanggapi/<int:id>', methods=['GET', 'POST'])
def tanggapi(id):
    if 'admin' not in session:
        return redirect(url_for('login'))
    if request.method == 'POST':
        tanggapan = request.form['tanggapan']
        cursor.execute("INSERT INTO tanggapan (pengaduan_id, tanggapan) VALUES (%s, %s)", (id, tanggapan))
        db.commit()
        return redirect(url_for('dashboard'))
